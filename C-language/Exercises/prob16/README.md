...
**Hint.** Primes get rarer as numbers get larger. The average gap between two consecutive primes near a large number $x$ is about $\ln(x)$, suggesting that the $n$-th prime number $p_n$ is roughly $n \cdot \ln(n)$. A safe explicit upper bound (by Rosser and Schoenfeld) is, for $n \ge 6$:
