...
#include <assert.h>
#include <stdint.h>
#include "../../../C-library/include/macros.h"

#define NA SIZE_MAX

//=================================================================
// Returns the maximum length of an increasing chain starting at
// index 'idx' if we still have 'rem' removals available.
size_t computeBest(size_t idx, size_t rem, size_t len, int *arr, 
                   size_t **memo) {
  if (idx >= len)
    return 0;
  ...
  for (size_t j = idx + 1; j < len; ++j) {
    if (arr[j] > arr[idx]) {
      ...
      if (removed <= rem) {
        size_t candidate = 1 + computeBest(j, rem - removed, len,
                                           arr, memo);
