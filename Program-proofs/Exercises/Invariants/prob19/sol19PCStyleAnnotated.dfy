/*  file: sol19PCStyleAnnotated.dfy
...
reads a
{
    ...
    //   = maximum (Max (a[i] + a[j] + a[h] | i,j,h: 0 ≤ i ≤ j < x-1 ∧ i ≤ h < x),
    //              Max (a[i] + a[x-1] + a[h] | i,h: 0 ≤ i ≤ x-1 ∧ i ≤ h < x))
    //     ( split the first maximum into h < x - 1 and h = x-1 )
    //   = maximum(
    //        maximum (Max (a[i] + a[j] + a[h] | i,j,h: 0 ≤ i ≤ j < x-1 ∧ i ≤ h < x-1),
    ...
    //   = maximum(
    //        maximum (S(a, x-1), a[x-1] + Max (a[i] + a[j] | i,j: 0 ≤ i ≤ j < x-1)),
    //        a[x-1] + Max (a[i] + a[h] | i,h: 0 ≤ i ≤ h < x))
    //     ( apply definition of U to the second term )
    //   = maximum (maximum (S(a, x-1), a[x-1] + U(a, x-1)),
    //              a[x-1] + Max (a[i] + a[h] | i,h: 0 ≤ i ≤ h < x))
    //     ( apply definition of U to the third term )
    //   = maximum (maximum (S(a, x-1), a[x-1] + U(a, x-1)), a[x-1] + U(a, x))
    //     ( since U(a, x) includes all pairs counted by U(a, x-1), the third term 
    //       dominates the second term and we can drop the second term )
    //   = maximum (S(a, x-1), a[x-1] + U(a, x))
  if x == 1 
  then 3 * a[0] 
...
reads a
{
    ...
    //   = maximum (U(a, x-1), 
    //              a[x-1] + maximum (Max (a[i] | i: 0 ≤ i < x-1), a[x-1]))
    //     ( apply definition of Z )
    //   = maximum (U(a, x-1), a[x-1] + maximum (Z(a, x-1), a[x-1]))
  if x == 1 
  then 2 * a[0] 
  else maximum (U(a, x-1), a[x-1] + maximum (Z(a, x-1), a[x-1]))
}
...
reads a
{
    ...
    //   = maximum (Max (a[i] | i: 0 ≤ i < x-1), Max (a[i] | i: i = x-1))
...
ensures  r == S(a, a.Length)
{
  ...
  decreases n - k
  {
      ...
      //   ( apply definition of Z to obtain 
      //     Z(a, k + 1) = maximum (Z(a, k), a[k]) )
      // 1 <= k < n ∧ s = S(a, k) ∧ u = U(a, k) ∧ Z(a, k + 1) = maximum (z, a[k])
    z := maximum(z, a[k]);
      // 1 <= k < n ∧ s = S(a, k) ∧ u = U(a, k) ∧ z = Z(a, k + 1)
      //   ( apply definition of U to obtain 
      //     U(a, k + 1) = maximum (U(a, k), a[k] + Z(a, k + 1)) )
      // 1 <= k < n ∧ s = S(a, k) ∧ z = Z(a, k + 1) 
      //   ∧ U(a, k + 1) = maximum (u, a[k] + z)
    u := maximum(u, a[k] + z);
      // 1 <= k < n ∧ s = S(a, k) ∧ z = Z(a, k + 1) ∧ u = U(a, k + 1)
      //   ( apply definition of S to obtain 
      //     S(a, k + 1) = maximum (S(a, k), a[k] + U(a, k + 1)) )
      // 1 <= k < n ∧ u = U(a, k + 1) ∧ z = Z(a, k + 1) 
      //   ∧ S(a, k + 1) = maximum (s, a[k] + u)
    s := maximum(s, a[k] + u);
