...
{
    // FIRST iteration: keep doubling divisor y 
    //   until it is larger than dividend x.
    ...
    decreases x - z   // J1 ∧ B1 ⇒ vf1 = x - z ≥ 0
  {
    ...
    i := i + 1;
      // z == 2^(i) * y ∧ x - z < V
      // J1 ∧ vf1 < V
      //   ( J1 is preserved, and the variant function vf1 has decreased. ) 
  }
    // J1 ∧ ¬B1
    // z == 2^i * y ∧ z > x
  ...
  while z != y        // B2
    ...
    invariant z == pow2(i) * y 
    decreases z - y   // J2 ∧ B2 ⇒ y < z, so vf2 = z - y > 0
  { 
      // J2 ∧ B2 ∧ vf2 = V
    ...
    i := i - 1;
      ...
      // In order to establish J2 after the loop body, we need to also 
      // guarantee that 0 ≤ r < z after the updates to z and q. This is done 
      // by adjusting r if r is larger than or equal to the new value of z.
    ...
    // x = q * z + r ∧ 0 ≤ r < z ∧ z = 2^i * y ∧ z - y < V
    // J2 ∧ vf2 < V
    //   ( J2 is preserved, and the variant function vf2 has decreased. )
  }
