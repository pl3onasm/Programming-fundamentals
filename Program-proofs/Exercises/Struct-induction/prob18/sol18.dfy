/*  file: sol18.dfy
    ...
    description: proof by structural induction of the correctness of
...
// non-empty. Once the input list is exhausted, the accumulator list  
  ...
  decreases xs
{
  if xs == Nil
  {
    ...
    assert ReverseAcc(xs, acc) == Append(Reverse(xs), acc) by
    {
      calc
      {
        ReverseAcc(xs, acc);
  ...
  else
  {
    ...
    ReverseAccProp(tail, Cons(x, acc));

      // Use associativity of Append to regroup the lists
    ...
    calc
    {
      ...
      == Append(Reverse(tail), Cons(x, acc));
        // Rewrite acc as Append(Nil, acc)
      == Append(Reverse(tail), Cons(x, Append(Nil, acc)));
        // Fold Append(Cons(x, Nil), acc)
      == Append(Reverse(tail), Append(Cons(x, Nil), acc));
        // Apply AppendAssociative in reverse direction
...
{
    // Use ReverseAccProp with acc = Nil
  ReverseAccProp(xs, Nil);

    // Use this lemma to remove the empty list appended to the result.
  AppendRightIdentity(Reverse(xs));
  ...
  calc
  {
    FastReverse(xs);
      // Unfold FastReverse
    == ReverseAcc(xs, Nil);
      // Apply ReverseAccProp with acc = Nil
    == Append(Reverse(xs), Nil);
      // Apply AppendRightIdentity
    == Reverse(xs);
  }
}
